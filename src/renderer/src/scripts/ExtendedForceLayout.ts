...
import { forceNBody } from '@antv/layout/lib/force/force-n-body'
import {
  CalcEdge,
  CalcEdgeData,
  CalcGraph,
...
export class ExtendedForceLayout implements LayoutWithIterations<ForceLayoutOptions> {
  ...
  tick(iterations = this.options.maxIteration || 1) {
    ...
    for (
      let i = 0;
      (this.judgingDistance > this.lastOptions.minMovement || i < 1) && i < iterations;
      i++
    ) {
  ...
  private formatOptions(options: ForceLayoutOptions, graph: Graph): FormatedOptions {
    ...
    formattedOptions.width =
      !propsWidth && typeof window !== 'undefined' ? window.innerWidth : (propsWidth as number)
    formattedOptions.height =
      !propsHeight && typeof window !== 'undefined' ? window.innerHeight : (propsHeight as number)
    if (!options.center) {
  ...
  private formatCentripetal(options: FormatedOptions, calcGraph: CalcGraph) {
    ...
    if (leafCluster && nodeClusterBy) {
      ...
      clusters =
        Array.from(new Set(calcNodes?.map((node) => node.data[nodeClusterBy] as string))) || []
      // @ts-ignore
    ...
    if (clustering && nodeClusterBy) {
      ...
      if (!clusters) {
        clusters = Array.from(
          new Set(calcNodes.map((node: Node) => node.data[nodeClusterBy] as string))
        )
      }
  ...
  public calAttractive(
    ...
    calcGraph.getAllEdges().forEach((edge) => {
 ...
 */
const getSameTypeLeafMap = (calcGraph: CalcGraph, nodeClusterBy: string): SameTypeLeafMap => {
  const calcNodes = calcGraph.getAllNodes()
  if (!calcNodes?.length) return {}
...
const getSameTypeNodes = (
  ...
  if (type === 'leaf') {
    sameTypeNodes = sameTypeNodes.filter(
      (item) =>
        calcGraph.getDegree(item.id, 'in') === 0 || calcGraph.getDegree(item.id, 'out') === 0
    )
  }
